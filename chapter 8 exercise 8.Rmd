...
```{r}
pred.tree = predict(tree.oj, newdata = OJ[-train,])
errtest.tree = sum(OJ[-train,] != pred.tree)/length(pred.tree)

pred.pruned = predict(tree.pruned, newdata = OJ[-train,])
errtest.pruned  = sum(OJ[-train,] != pred.pruned)/length(pred.pruned)
